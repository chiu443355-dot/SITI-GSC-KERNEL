...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all siti.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*siti\.sh$/)) {
        return true;
      }

      // Allow all sitiagent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*sitiagent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@siti.sh" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@siti.sh" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@siti.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@siti.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
