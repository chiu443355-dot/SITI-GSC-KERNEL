...
export default function DataInjection({
  ...
  const handleUpload = async (e) => {
    ...
    setUploadMsg(null);
    setUploadError(null);
    onCalibrating?.(true);          // show full-screen overlay
    try {
      ...
      const [res] = await Promise.all([
        ...
        new Promise(r => setTimeout(r, 2800)), // min 2.8 s overlay
      ]);
      ...
      setUploading(false);
      onCalibrating?.(false);       // hide overlay
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ["Instability Index Φ(ρ)", phi.toFixed(4), phi < 0.2 ? "CRITICAL" : phi < 0.5 ? "DEGRADED" : "HEALTHY"],
    ...
    [
      ...
      `2. Sigmoidal Decay:     Φ(ρ) = 1/(1+exp(-20(ρ-${(kState?.critical_rho ?? 0.85).toFixed(2)}))) = ${phi.toFixed(4)}`,
      `3. Priority Leakage:    L = $1.20 + $2.74 = $3.94 × ${irp?.failure_count ?? 0} = $${irp?.leakage_total?.toFixed(2) ?? "0.00"}`,
    ...
    doc.text("This audit has been processed by SITI Intelligence — MIMI Intelligence Engine v2.0,", 105, y + 18, { align: "center" });
    doc.text("in accordance with Mission LiFE (Lifestyle for Environment) ESG compliance framework.", 105, y + 24, { align: "center" });
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontSize: 9, color: "#888", marginBottom: 12, lineHeight: 1.6 }}>
          Upload a new shipment dataset (CSV). MIMI Kernel wipes historical weights, runs fresh
          logistic regression, and auto-recalculates ρ_critical.
          <span style={{ color: "#64D2FF" }}> GHOST MODE: data lives in volatile memory only.</span>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <input
            ref={fileRef} type="file" accept=".csv" onChange={handleUpload}
          ...
          <div style={{ fontSize: 9, color: "#777" }}>
            Fuzzy mapping active — messy headers auto-detected.
          </div>
        </div>
        {uploadMsg && (
      ...
      <div>
        ...
        <div style={{ fontSize: 9, color: "#888", marginBottom: 12, lineHeight: 1.6 }}>
          Board-ready PDF: <em>SITI: Forensic State Audit [Case #02028317]</em> — full MIMI
          Kernel analysis, Inverse Reliability findings, &amp;{" "}
          <span style={{ color: "#32D74B" }}>Mission LiFE ESG Compliance Certification</span>.
        </div>
        ...
        <div style={{ marginTop: 10, padding: "8px", background: "#060606", border: "1px solid #141414", fontSize: 8, color: "#777", lineHeight: 1.8 }}>
          ...
          {[
            ...
            "Warehouse Utilization Breakdown", "Inverse Reliability Paradox Table (Top 15)",
            "Kalman Filter State Analysis", "Mission LiFE ESG Compliance Tag",
          ].map(item => (
      ...
      <div>
        <div style={{ fontSize: 9, color: "#D4D4D8", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 8 }}>
          LIVE TELEMETRY — VIRTUAL UNIT STREAM
        </div>
        <div style={{ fontSize: 9, color: "#888", marginBottom: 12, lineHeight: 1.6 }}>
          Injects 100 virtual shipment units every 10 s based on the current hub ρ distribution.
          <span style={{ color: "#64D2FF" }}> No CSV needed — real-time diversion demo.</span>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <button
            data-testid="live-stream-toggle"
          ...
          >
            <span
              className={isStreaming ? "stream-dot-live" : ""}
              style={{
                display: "inline-block", width: 6, height: 6, borderRadius: "50%",
                background: isStreaming ? "#32D74B" : "#555",
                flexShrink: 0,
              }}
            />
            {isStreaming ? "HALT STREAM" : "INITIATE LIVE STREAM"}
          </button>
          {isStreaming && (
            <div style={{ fontSize: 9, color: "#32D74B", fontFamily: "JetBrains Mono", letterSpacing: "0.08em" }}>
              100 UNITS / 10s
            </div>
          )}
        </div>
        ...
        {isStreaming && (
          <div style={{
            marginTop: 8, padding: "6px 10px", background: "#001A0A",
            border: "1px solid #32D74B", fontSize: 8, color: "#32D74B", letterSpacing: "0.08em",
          }}>
            STREAM ACTIVE — MIMI KERNEL PROCESSING VIRTUAL TELEMETRY
          </div>
        )}
        <div style={{ marginTop: 10, fontSize: 8, color: "#666", lineHeight: 1.8 }}>
          <div style={{ color: "#CCCCCC", marginBottom: 4, letterSpacing: "0.1em" }}>STREAM BEHAVIOUR:</div>
          {[
            "Units generated ~ρ distribution ± Gaussian noise",
            "~25% of predicted-late units auto-diverted by SITI",
            "Revenue counter increments live",
            "Kalman filter re-estimates T+1 each cycle",
          ].map(item => (
            <div key={item} style={{ display: "flex", gap: 6 }}>
