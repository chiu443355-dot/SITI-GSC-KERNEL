...
class TenantKernel:
    ...
    def _update_hub(self, rec: dict):
        ...
        lam = float(rec.get("arrival_rate", 0))
        mu  = float(rec.get("service_rate", 1))
...
@app.get("/")
def home():
    return jsonify({"status": "SITI Kernel Online", "version": "2.0"})

@app.get("/health")
def health():
    return jsonify({"status": "alive", "kernel": "SITI GSC v2.0", "timestamp": datetime.utcnow().isoformat()})

@app.post("/api/kernel/analyze")
def analyze_logistics():
    """OpenRouter AI analysis of a logistics failure in plain English."""
    ...
    try:
        response = httpx.post(
            ...
            json={
                ...
                "messages": [{"role": "user", "content": f"Explain this logistics delay in plain English for an operations manager: {data}"}]
            },
            timeout=30.0
        )  # ← closing ) was the original bug — was } before
...
@require_api_key
def genius_reset():
    """Upload CSV, wipe kernel state, re-ingest dataset."""
    ...
    if not f.filename.lower().endswith(".csv"):
        return jsonify({"error": "Unsupported format. Please upload a .csv file."}), 400
    ...
    if critical_hubs:
        ...
        msg = (
            ...
            f"Load factor ρ = {hub['rho']} (system over capacity).\n"
            f"Queue depth: {hub['queue_depth']} shipments.\n"
            f"IRP Score: {hub['irp_score']}/10. Reroute immediately."
        )
...
def predict():
    ...
    if predicted and predicted[-1] > 0.90:
        ...
        msg = (
            f"SITI KERNEL — {hub_id} delay probability critical.\n"
            f"Kalman predicts {predicted[-1]*100:.1f}% delay within 5 ticks.\n"
            f"Queue: {int(hub_stats.get('queue_depth', 0))} shipments. Intervene now."
        )
        send_sms(msg)
